...
from colormath.color_objects import LabColor, HSLColor, BaseRGBColor, AdobeRGBColor
...
def convert_color_space(cielab):
    if settings.color_space == 'cielab':
        return cielab
    lab = LabColor(*cielab)
    if settings.color_space == 'hsl':
        hsl_color = convert_color(lab, HSLColor, through_rgb_type=AdobeRGBColor)
        _color = np.array([hsl_color.hsl_h, hsl_color.hsl_s, hsl_color.hsl_l])
    elif settings.color_space == 'rgb':
        _color = color.lab2rgb(cielab[None, None])[0, 0]
        # rgb_color = convert_color(lab, AdobeRGBColor)
        # _color = np.array([rgb_color.rgb_r, rgb_color.rgb_g, rgb_color.rgb_b])
...
class WCSDataset:
    ...
    def get_features(self, cnum):
        return convert_color_space(self.chips[cnum - 1])
